...
위의 내용과 같이 지원합니다. 거의 모든 브라우저에서 `defer` 속성을 지원합니다.

# 무엇을 사용할 것인가
브라우저는 기본적으로 `<script>` 태그를 만나면 파싱을 중단하고 스크립트를 다운, 파싱, 실행 과정을 진행합니다. `<script>` 태그가 `<body>` 태그가 닫히기 바로 직전에 선언되었다면, 이미 모든 HTML 파싱이 종료된 상태이기 때문에, `async` 속성을 사용하든 `defer` 속성을 사용하든 크게 의미가 없게 됩니다.
...
`async`는 스크립트 리소스 다운로드가 완료되면 파싱을 중단하고 스크립트가 실행되기 때문에, 어느 시점에 HTML 파싱이 중단 될지 보장할 수 없습니다. DOM에 종속성이 없어서  HTML 파싱 중 어느 시점에 스크립트가 실행되든지 상관 없을 경우 `async` 속성을 사용하는 것이 좋습니다.
...
## `<noscript>`
최신 브라우저는 자바스크립트를 지원하지 않는 경우를 찾아 볼 수 없지만, 자바스크립트 실행이 비활성화 되어있거나 자바스크립트 기능이 없는 구형 브라우저의 경우 `<noscript>` 태그 내의 HTML이 파싱되어 화면에 그려집니다.
