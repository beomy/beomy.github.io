...
# HTTP란
HTTP(Hyper Text Transfer Protocol)는 W3(World Wide Web)에서 정보를 주고 받을 수 있는 프로토콜로, HTTP는 클라이언드와 서버 사이에 이루어지는 요청/응답(request/response) 프로토콜입니다.

예를 들어 웹 브라우저가 HTTP를 통해 서버로부터 웹페이지(HTML)나 이미지를 요청하면 서버는 이 요청에 응답하여 필요한 정보를 전달하게 됩니다.
...
# HTTP 요청 메시지
클라이언트가 서버에게 보내는 요청 메시지의 구조와 메시지 구성에 포함 되는 HTTP 메서드에 대해 좀 더 자세히 살펴보겠습니다.
