...
이번 포스트에서는 웹 취약성을 노리는 XSS와 XSRF(CSRF)와 공격에 대해 살펴보도록 하겠습니다.
...
## 대응 방법
XSS 공격을 막기 위해서는 페이지에서 공격자가 입력한 스크립트를 실행하지 못하게 해야 합니다. 스크립트를 실행하지 못하게 하려면, 공격자의 스크립트 입력을 막거나 브라우저가 스크립트를 실행하지 못하게 공격자의 스크립트 값을 변경해야 합니다. XSS 공격은 보통 프론트엔드 측에서 대응해 줍니다.
