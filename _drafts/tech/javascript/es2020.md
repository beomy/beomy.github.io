...
|name|description|
|:----:|:---:|
|`Promise.allSettled`|모든 작업이 `resolve` 혹은 `reject`될 경우 종료됨|
...
# `globalThis`
브라우저에서 전역 객체는 `window`이고, Node.js의 전역 객체는 `global` 런타임 마다 전역 객체가 달라서, 아래와 같이 전역 객체를 가져오는 공통 코드를 만들어야 합니다.
