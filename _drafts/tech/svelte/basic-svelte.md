...
이번 포스트에서는 Svelte의 기본 문법에 대해 이야기 합니다.

# data 정의
화면에 반응형으로 표시 되는 데이터를 정의하는 방법에 대해 이야기 해보도록 하겠습니다. Vue.js에서는,
...
위의 코드와 같이 반응형으로 표시 되는 데이터를 정의할 수 있습니다. Svelte에서는,
...
위의 코드와 같이 반응형으로 표시 되는 data를 정의할 수 있습니다. 단순히 변수를 정의 하듯 사용하면 됩니다. Vue.js 보다 코드가 간결하고 직관적입니다.

Vue.js의  {% raw %}`{{...}}`{% endraw %} 관로 안에 자바스크립트 사용이 가능한 것 처럼, Svelte의 `{...}` 괄호 안에는 자바스크립트 사용이 가능합니다.
...
Svelte는 반응형 데이터를 HTML에 선언했던 것과 동일하게 HTML에 `{...}`을 사용하여 동적 속성을 정의할 수 있습니다. Vue.js에서 동적 송성을 `:src` 등으로 표현하는 것과 달리 Svelte는 `{...}`을 사용하여 동적 속성을 정의 할 수 있어 코드에 통일성이 있습니다.

## 속성 약어
Svelte는 간략한 코드를 더욱 간략하게 할 수 있는 속성을 약어로 표시 할 수 있는 기능을 제공합니다. 약어는 아래 코드와 같이 사용할 수 있습니다.
...
Vue.js와 Svelte의 차이는 Vue.js에서는 `<style scoped>`이라고 명시를 해 줘야 로컬(local) 스타일이 되지만, Svelte에서는 기본이 지역입니다. Svelte에서 글로벌(global) 스타일을 지정하기 위해서는 `<style global>`로 명시 해 주어야 합니다.

# 컴포넌트 정의
컴포넌트를 정의하고 사용하는 방법도 Vue.js와 유사합니다. 컴포넌트는 아래 코드와 같이 정의 되었을 때,
...
스타일 정의에서 말씀드렸던 것과 같이 Svelte는 스타일 적용 범위 기본이 로컬이기 때문에, 컴포넌트 안의 `<p>` 태그에 스타일이 적용되지 않은 것을 확인 할 수 있습니다.
