...
# 트렌지션 종류
Svelte는 7개의 트렌지션을 제공합니다. 자세한 내용은 [https://svelte.dev/docs#svelte_transition](https://svelte.dev/docs#svelte_transition)를 참고 부탁드립니다. 가장 마지막에 이야기 할 `crossfade` 트렌지션은 조금 다른 방법으로 사용해야 합니다.
...
## `crossfade` 트렌지션
`crossfade` 트렌지션은 밑의 코드와 같이 사용해야 합니다. 예제 코드와 실핼 결과를 먼저 보도록 하겠습니다.
...
### 실행 화면
![crossfade 트렌지션](/assets/img/posts/svelte/crossfade_transition.gif)

### 사용방법
`crossfade` 트렌지션의 사용방법은 다른 트렌지션을 사용하는 방법과 조금 다릅니다. 사용방법을 순서대로 살펴보겠습니다.

- `import { crossfade } from 'svelte/transition';`로 `crossfade` 함수를 가져옵니다.
- `const [send, receive] = crossfade(params)`로 함수를 호출합니다.
- `crossfade` 함수의 파마리터로 `duration`, `delay`, `fallback` 속성을 포함한 객체를 전달합니다.
  - `duration` (number \| function, default: `d => Math.sqrt(d) * 30`): 단위는 ms로 트렌지션 지속시간입니다. 함수일 경우 `d`는 거리입니다.
  - `delay` (number, default: 0): 단위는 ms로 설정한 시간 후에 트렌지션이 시작됩니다.
  - `easing` (function, default: cubicOut): easing 함수입니다. [https://svelte.dev/docs#svelte_easing](https://svelte.dev/docs#svelte_easing) 참고 바랍니다.
  - `fallback` (function): 이동할 대상이 없는 경우 실행되는 트렌지션을 정의합니다.
- `crossfade` 함수의 리턴 값은 `[send, receive]` 형태의 배열입니다.
  - `send`: 내보내는 트렌지션입니다. {% raw %}`out:send="{{key: todo.id}}"`{% endraw %} 형태로 사용해야 합니다. 유티크한 key를 가진 객체를 파라미터로 전달해야 합니다.
  - `receive` 받는 트렌지션입니다. {% raw %}`in:receive="{{key: todo.id}}"`{% endraw %} 형태로 사용해야 합니다. `send` 트렌지션과 동일하게 유니크한 key를 가진 객체를 파라미터로 전달해야 합니다.

받는 대상, 보내는 대상이 없는 경우 `fallback`에 정의한 트렌지션이 동작하게 되는데, `fallback`에 트렌지션을 정의하는 방법은 [/tech/svelte/transitions/#커스텀-트렌지션-만들기](/tech/svelte/transitions/#커스텀-트렌지션-만들기)에서 자세히 이야기 하도록 하고 지금은 넘어가도록 하겠습니다.

`send`와 `receive` 트렌지션을 사용할 때, 받을 때 사용한 `in`과 내보낼 때 사용한 `out`에 대해 살펴보겠습니다.
...
`css`와 `tick` 속성을 사용하면 커스텀한 트렌지션을 만들 수 있습니다. `tick` 속성은 매 `tick` 마다 호출되는 콜백함수이기 때문에 `tick`을 사용하여 만들어진 트렌지션은 매끄러운 애니메이션이 동작하지 않을 수 있습니다. 커스텀한 트렌지션을 만들 때는 `css` 속성을 사용하여 만드는 것을 권장합니다.

`css`와 `tick` 속성은 이 후 내용에서 더 자세히 살펴보도록 하겠습니다.

## CSS로 트렌지션 만들기
트렌지션 함수의 `css` 속성을 사용하여 트렌지션 함수를 만들어 보도록 하겠습니다.
...
위의 코드를 살펴보면, `css` 속성에 `t`의 변화에 따라 `transform`과 `color`의 CSS 문자열을 리턴하는 `spin` 트렌지션 함수를 볼 수 있습니다.

## JavaScript로 트렌지션 만들기
트렌지션 함수의 `tick` 속성을 사용하여 트렌지션 함수를 만들어 보도록 하겠습니다.

일반적으로는 CSS를 사용해서 트렌지션을 만들어야 하지만 자바스크립트를 사용해야 하는 효과가 있을 수 있습니다. 자바스크립트를 사용한 타자효과를 표현한 트렌지션 예제를 살펴보겠습니다.
...
![타자기 트렌지션](/assets/img/posts/svelte/typewriter_transition.gif)

# 트렌지션 이벤트
Svelte는 트렌지션이 언제 시작되고 끝이 나는지 알려주는 트렌지션 이벤트를 제공합니다. 사용하는 방법은 다른 DOM 이벤트와 동일합니다. 예제 코드를 살펴보도록 하겠습니다.
...
- `introstart`: 요소가 추가되는 트렌지션의 시작 이벤트입니다.
- `outrostart`: 요소가 제거되는 트렌지션의 시작 이벤트입니다.
- `introend`: 요소가 추가되는 트렌지션의 종료 이벤트입니다.
- `outroend`: 요소가 제거되는 트렌지션의 종료 이벤트입니다.

# `local` 수식어
Svelte의 트렌지션은 `local` 수식어를 제공합니다. `local` 수식어를 사용하면 상위 블록에 요소가 추가될 경우에만 트렌지션이 동작합니다. 말로는 이해하기 어려우니 예제를 하나 살펴보도록 하겠습니다.
...
위의 코드와 같이 `transition:slide|local` 사용하여 `local` 수식어를 사용할 수 있습니다. 위의 코드는 `local` 수식어를 사용했기 때문에 상위 블록에 요소인 `{#each }` 블록에 요소가 추가 될 때에만 트렌지션이 동작합니다. `local` 수식어 사용한 경우와 사용하지 않은 경우를 비교해 보겠습니다.
...
- `local` 수식어 미사용: 전체 리스트가 나타날 때와 목록이 추가 될 때 트렌지션이 동작하는 것을 볼 수 있습니다.
- `local` 수식어 사용: 전체 리스트가 나타날 때 트렌지션이 동작하지 않지만 목록이 추가 될 때에는 트렌지션이 동작하는 것을 볼 수 있습니다.
