...
# 라이프 사이클 스크립트 종류
`NPM`은 `prepare`, `prepublish`, `prepublishOnly`, `prepack`, `postpack`, `dependencies` 6개의 라이프 사이클 스크립트가 있습니다. 반면에 `Yarn 2+`는 `NPM`과 달리 `prepack`, `postpack`, `prepublish`, `postintall` 4개의 라이프 사이클 스트립트만 존재합니다. 각각의 스크립트는 `NPM` 라이프 사이클 스크립트와 동일한 시점에 호출됩니다. 라이프 사이클 스크립트를 하나씩 살펴보도록 하겠습니다.
...
> **`Yarn 2+` 라이프 사이클 실행 순서**
> 
> `Yarn 2+`에서 명령어별 라이프 사이클 실행 순서는 `NPM`의 명령어별 라이프 사이클 실행 순서에서 `prepack`, `postpack`, `prepublish`, `postintall`이 4개의 스크립트만 호출됩니다.
