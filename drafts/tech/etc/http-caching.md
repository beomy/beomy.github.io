...
- **검색**: 로컬 복사본이 있는지 검사하고, 사본이 없다면 서버에서 문서를 가져온 후 로컬에 저장합니다.
- **신선도 검사**: 캐시된 사본이 신선한지 검사하고 신선하지 않다면 변경 사항이 있는지 서버에 물어봅니다.
...
# 캐시 신선도
캐시된 사본은 항상 서버의 문서와 일치하지 않습니다. 서버의 데이터들이 언제든 바뀔 수 있기 때문에 캐시 된 데이터는 서버의 데이터와 일치하도록 관리되어야 합니다. 프록시 캐시는 문서 만료와 서버 재검사 메커니즘으로 캐시 신선도를 유지하게 됩니다.
